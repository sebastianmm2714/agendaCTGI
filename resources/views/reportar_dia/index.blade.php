        ...
        <div class="container-fluid px-md-5 overflow-hidden">
            ...
            <div class="mt-3">
                <div class="card border shadow-sm mb-4">
                    <div class="card-body d-flex flex-column flex-sm-row align-items-center justify-content-center py-3 text-center text-sm-start">
                        <img src="{{ asset('images/sena/logo250.png') }}" alt="SENA"
                            style="height: 60px;" class="mb-2 mb-sm-0 me-sm-3">
            ...
            <div class="card border-0 shadow-sm rounded-4">
                ...
                <div class="card-body p-0">
                        ...
                        <form action="{{ route('reportar-dia') }}" method="GET" class="row g-3 align-items-center">
                            <div class="col-12 col-md-6">
                                <div class="input-group input-group-lg shadow-sm">
                                    ...
                                    @if(request('search'))
                                        <a href="{{ route('reportar-dia') }}" class="btn btn-white border-start-0 text-muted"
                            ...
                            <div class="col-12 col-md-2 position-relative">
                                ...
                                <div class="custom-select-trigger d-flex justify-content-between align-items-center shadow-sm w-100"
                                    tabindex="0" style="height: 46px; padding: 0.5rem 1rem;">
                                    <span id="estado_trigger_text"
                                        class="text-truncate fw-bold {{ request('estado_id') ? 'text-dark' : 'text-muted' }}" style="font-size: 0.85rem; text-transform: uppercase;">
                                        {{ $estadoActivoTexto }}
                                    </span>
                                ...
                                <div class="custom-select-dropdown" style="display:none;">
                                    <div class="custom-select-option {{ !request('estado_id') ? 'selected-item' : '' }}"
                                        data-value="" data-text="TODOS">
                                        TODOS
                                    </div>
                                    @foreach($estados as $estado)
                                        @php $n = str_replace('_', ' ', $estado->nombre); @endphp
                                        <div class="custom-select-option {{ request('estado_id') == $estado->id ? 'selected-item' : '' }}"
                                            data-value="{{ $estado->id }}" data-text="{{ $n }}">
                                            {{ $n }}
                                        </div>
                                    @endforeach
                                </div>
                            ...
                            <div class="col-6 col-md-2">
                                <select name="per_page" class="form-select rounded-pill shadow-sm bg-white fw-bold text-muted" onchange="this.form.submit()" title="Registros por página" style="font-size:0.85rem; height: 46px; border: 1px solid #dee2e6;">
                            ...
                            <div class="col-6 col-md-2">
                                <button type="submit"
                                    class="btn btn-success w-100 rounded-pill fw-bold py-2 shadow-sm" style="height: 46px;">
                        ...
                        <table class="table table-hover align-middle mb-0">
                            <thead class="bg-light">
                                <tr class="text-dark">
                                    ...
                                    <th class="py-3 text-center text-muted small text-uppercase" style="width: 250px;">
                                        Estado</th>
                                    <th class="text-center py-3 text-muted small text-uppercase" style="width: 250px;">
                                        Acciones</th>
                                </tr>
                            </thead>
                            ...
                            <tbody>
                                @forelse ($agendas as $agenda)
                                    <tr>
                                        <td
                                            class="ps-4 fw-bold text-success">
                                            #{{ $agenda->id }}
                                        </td>
                                        ...
                                        <td style="max-width: 300px;">
                                            <span class="text-muted small text-uppercase mb-0"
                                        ...
                                        <td class="text-center">
                                            ...
                                            @if($agenda->estado && ($agenda->estado->nombre == 'CORRECCIÓN' || ($agenda->estado->nombre == 'ENVIADA' && $agenda->observaciones_finanzas)))
                                                <div class="alert alert-danger mb-0 py-1 px-2 shadow-sm d-inline-block text-start clickable-alert"
                                                    ...
                                                    data-bs-toggle="modal" data-bs-target="#modalInfo{{ $agenda->id }}"
                                                    title="Clic para ver detalles">
                                                    <strong class="d-block text-uppercase small"><i
                                                            class="fas fa-undo-alt me-1"></i> Devuelta</strong>
                                                    <span
                                                        class="text-dark">{{ Str::limit($agenda->observaciones_finanzas, 30) }}</span>
                                                </div>
                                            @else
                                                @php
                                        ...
                                        <td class="text-center px-4">
                                            <div class="d-flex justify-content-center align-items-center gap-2">
                                                ...
                                                @php
                                                    $estadoNombre = strtoupper(trim($agenda->estado->nombre ?? ''));
                                                    // Forzamos true si es borrador o corrección de forma simple
                                                    $isEditable = ($estadoNombre == 'BORRADOR' || $estadoNombre == 'CORRECCIÓN' || ($estadoNombre == 'ENVIADA' && $agenda->observaciones_finanzas));
                                                @endphp

                                                @if($isEditable)
                                                    {{-- Botón Editar/Corregir --}}
                                                    <a href="{{ route('formulario', $agenda->id) }}"
                                                        class="btn btn-sm {{ $estadoNombre == 'BORRADOR' ? 'btn-outline-success' : 'btn-warning text-dark' }} rounded-pill px-3 fw-bold shadow-sm d-flex align-items-center"
                                                        title="{{ $estadoNombre == 'BORRADOR' ? 'Editar los datos básicos' : 'Corregir la agenda devuelta' }}">
                                                        <i class="fas {{ $estadoNombre == 'BORRADOR' ? 'fa-edit' : 'fa-exclamation-triangle' }} me-1"></i>
                                                        {{ $estadoNombre == 'BORRADOR' ? 'Editar' : 'Corregir' }}
                                                    </a>
                                                    ...
                                                    <a href="{{ route('reportar-dia.show', $agenda->id) }}"
                                                        ...
                                                        {{ auth()->user()->role == 'funcionario' ? 'Reportes' : 'Reportar' }}
                                                    </a>
                                                @endif
                                                ...
                                                @if(!$agenda->estado || ($agenda->estado && in_array($agenda->estado->nombre, ['BORRADOR', 'CORRECCIÓN'])) || ($agenda->estado && $agenda->estado->nombre == 'ENVIADA' && $agenda->observaciones_finanzas))
                                                    ...
                                                    <form action="{{ route('reportar-dia.enviar', $agenda->id) }}" method="POST"
                                                        ...
                                                        <button type="submit"
                                                            ...
                                                            data-total="{{ $diasTotal }}" data-reportados="{{ $diasReportados }}">
                                                ...
                                                <a href="{{ route('agenda.pdf', $agenda->id) }}" target="_blank"
                                                    ...
                                                    style="min-width: 60px;" title="Ver PDF">
                    ...
                    <div class="px-4 py-4 bg-light border-top d-flex justify-content-center align-items-center">
                        <div
                            class="pagination-container shadow-sm p-1 bg-white rounded-pill d-inline-block custom-pagination">
