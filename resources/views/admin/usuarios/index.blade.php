    ...
    <div class="container-fluid px-4">
        <div class="d-flex align-items-center justify-content-between mb-4 animate__animated animate__fadeIn">
            <div>
                ...
                <p class="text-muted mb-0">Control de Personal y asignación de responsables</p>
            </div>
                ...
                <form action="{{ route('admin.usuarios.index') }}" method="GET" id="searchForm">
                    <div class="row g-3 align-items-center">
                        ...
                        <div class="col-md-5">
                            <div class="input-group shadow-sm rounded-pill overflow-hidden">
                                ...
                                <input type="text" name="search" value="{{ request('search') }}"
                                    class="form-control border-start-0" placeholder="Buscar por nombre o documento...">
                        ...
                        <div class="col-md-4 d-flex align-items-center gap-2 justify-content-end">
                            <select name="per_page" class="form-select rounded-pill w-auto shadow-sm"
                            ...
                            <button type="button" class="btn btn-success rounded-pill px-3 fw-bold shadow-sm"
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        <thead class="bg-light">
                            <tr>
                                ...
                                <th class="py-3 text-muted small fw-bold text-center">ROL</th>
                                <th class="py-3 text-muted small fw-bold text-center">CATEGORÍA</th>
                                ...
                                <tr>
                                    <td class="ps-4">
                                        <div class="fw-bold text-dark"><span
                                                class="text-muted small">{{ $user->tipo_documento }}</span>
                                            {{ $user->numero_documento }}</div>
                                        <div class="text-uppercase small text-muted">{{ $user->name }}</div>
                                    </td>
                                    <td class="text-center">
                                        <div class="small mt-1"><span
                                                class="badge bg-light text-dark border">{{ $user->role }}</span></div>
                                    </td>
                                    <td class="text-center">
                                        <span class="badge rounded-pill px-3 py-2 fw-normal"
                                            style="background-color: #f0f7ff; color: #007bff; border: 1px solid #d0e7ff;">
                                            {{ $user->categoria->nombre ?? 'N/A' }}
                                        </span>
                                    </td>
                                    <td>
                                        <div class="small"><strong>Sup:</strong>
                                            {{ $user->supervisor->nombre ?? 'No asignado' }}</div>
                                        <div class="small mt-1"><strong>Ord:</strong>
                                            {{ $user->ordenador->nombre ?? 'No asignado' }}</div>
                                    </td>
                                    <td class="pe-4 text-end">
                                        ...
                                        <form action="{{ route('admin.usuarios.destroy', $user->id) }}" method="POST"
                                            class="d-inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="btn btn-link text-danger p-1 btn-confirm-delete"
                                                data-title="¿Eliminar Usuario?"
                                                data-text="Se eliminará el registro de '{{ $user->name }}'.">
                                            ...
                                            <form action="{{ route('admin.usuarios.update', $user->id) }}" method="POST">
                                                ...
                                                <div class="modal-body p-4">
                                                    <div class="row g-3">
                                                        ...
                                                        <div class="col-md-12">
                                                            <label class="form-label fw-bold">Cargo / Función</label>
                                                        ...
                                                        <div class="col-md-3">
                                                            ...
                                                            <input type="text" name="numero_documento"
                                                                ...
                                                                value="{{ $user->numero_documento }}" required>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <label class="form-label fw-bold">Salario / Honorarios</label>
                                                            <input type="number" name="salario_honorarios"
                                                                class="form-control rounded-3"
                                                                value="{{ $user->salario_honorarios }}" step="0.01">
                                                        </div>
                                                        <div class="col-md-6">
                                                            <label class="form-label fw-bold">Nro. Cuenta - Tipo</label>
                                                            <input type="text" name="numero_cuenta_tipo"
                                                                class="form-control rounded-3"
                                                                value="{{ $user->numero_cuenta_tipo }}"
                                                                placeholder="Ej: 91255755752 Ahorros Bancolombia">
                                                        </div>
                                                        <div class="col-md-6">
                                                            ...
                                                            <select name="role" class="form-select rounded-3" required>
                                                                ...
                                                                <option value="supervisor_contrato" {{ $user->role == 'supervisor_contrato' ? 'selected' : '' }}>
                                                                    Supervisor (Líder de Proceso)</option>
                                                                <option value="ordenador_gasto" {{ $user->role == 'ordenador_gasto' ? 'selected' : '' }}>
                                                                    Ordenador (Líder de Proceso)</option>
                                                        ...
                                                        <div class="col-md-12">
                                                            ...
                                                            <select name="categoria_personal_id" class="form-select rounded-3"
                                                                required>
                                                                @foreach($categorias as $cat)
                                                                    <option value="{{ $cat->id }}" {{ $user->categoria_personal_id == $cat->id ? 'selected' : '' }}>
                                                                        {{ $cat->nombre }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
                                                        ...
                                                        <div class="col-md-6">
                                                            ...
                                                            <input type="text" name="numero_contrato"
                                                                class="form-control rounded-3"
                                                                value="{{ $user->numero_contrato }}" required>
                                                        </div>
                                                        <div class="col-md-3">
                                                            <label class="form-label fw-bold">Año</label>
                                                            <input type="number" name="anio_contrato"
                                                                class="form-control rounded-3"
                                                                value="{{ $user->anio_contrato }}" required>
                                                        </div>
                                                        <div class="col-md-3">
                                                            <label class="form-label fw-bold">Vencimiento</label>
                                                            <input type="date" name="fecha_vencimiento"
                                                                class="form-control rounded-3"
                                                                value="{{ $user->fecha_vencimiento }}" required>
                                                        </div>
                                                        <div class="col-md-12">
                                                            <label class="form-label fw-bold">Objeto Contractual</label>
                                                            <textarea name="objeto_contractual" class="form-control rounded-3"
                                                                rows="2" required>{{ $user->objeto_contractual }}</textarea>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <label class="form-label fw-bold text-success">Supervisor
                                                                Asignado</label>
                                                            <select name="supervisor_id"
                                                                class="form-select rounded-3 border-success">
                                                        ...
                                                        <div class="col-md-6">
                                                            <label class="form-label fw-bold text-primary">Ordenador de
                                                                Gasto</label>
                                                            <select name="ordenador_id"
                                                ...
                                                <div class="modal-footer border-0 p-4 pt-0">
                                                    <button type="submit"
                                                        class="btn btn-primary w-100 rounded-pill fw-bold py-2">Guardar
                                                        Cambios</button>
                                                </div>
                                            </form>
                                        </div>
                    ...
                    @if($users->total() > $users->perPage())
                        <div class="card-footer bg-white border-top-0 py-4 d-flex justify-content-center">
                            <div
                                class="pagination-container shadow-sm p-1 bg-light rounded-pill d-inline-block custom-pagination">
                                {{ $users->links() }}
                            </div>
                        </div>
                    @endif
                ...
                <form action="{{ route('admin.usuarios.store') }}" method="POST">
                    ...
                    <div class="modal-body p-4">
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Nombre Completo</label>
                                <input type="text" name="name" class="form-control rounded-3" placeholder="Ej: JUAN PEREZ"
                                    required>
                            </div>
                            <div class="col-md-12">
                                <label class="form-label fw-bold">Cargo / Función</label>
                                <input type="text" name="cargo" class="form-control rounded-3 text-uppercase"
                                    placeholder="Ej: COORDINADOR ACADÉMICO">
                            </div>
                            <div class="col-md-2">
                                <label class="form-label fw-bold">Tipo Doc.</label>
                            ...
                            <div class="col-md-4">
                                <label class="form-label fw-bold">Número de Documento</label>
                                <input type="text" name="numero_documento" id="doc_input" class="form-control rounded-3"
                                    placeholder="Sin puntos ni comas" required>
                                <div id="doc_feedback" class="small mt-1 d-none"></div>
                                <div id="password_generator_container" class="mt-2 d-none">
                                    <label class="form-label fw-bold text-success small mb-1"><i class="fas fa-key me-1"></i>Contraseña Sugerida</label>
                                    <div class="input-group input-group-sm">
                                        <input type="text" name="password" id="generated_password" class="form-control bg-light border-success text-success fw-bold" readonly>
                                        <button type="button" class="btn btn-success" id="btn_copy_password" title="Copiar contraseña">
                                            <i class="fas fa-copy"></i>
                                        </button>
                                    </div>
                                    <small id="copy_tooltip" class="text-success d-none"><i class="fas fa-check me-1"></i>¡Copiado!</small>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Salario / Honorarios</label>
                                <input type="number" name="salario_honorarios" class="form-control rounded-3"
                                    placeholder="Ej: 5000000" step="0.01">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Nro. Cuenta - Tipo</label>
                                <input type="text" name="numero_cuenta_tipo" class="form-control rounded-3"
                                    placeholder="Ej: 91255755752 Ahorros Bancolombia">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Rol en Sistema</label>
                                <select name="role" class="form-select rounded-3 role-select" required>
                            ...
                            <div class="col-md-12">
                                ...
                                <div id="cat_feedback" class="small mt-1 d-none text-danger">Por favor seleccione una
                                    categoría</div>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Número de Contrato</label>
                                <input type="text" name="numero_contrato" class="form-control rounded-3"
                                    placeholder="Ej: 123-2024" required>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label fw-bold">Año</label>
                                <input type="number" name="anio_contrato" class="form-control rounded-3" placeholder="2024"
                                    required>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label fw-bold">Vencimiento</label>
                            ...
                            <div class="col-md-12">
                                <label class="form-label fw-bold">Objeto Contractual</label>
                                <textarea name="objeto_contractual" class="form-control rounded-3" rows="2"
                                    placeholder="Descripción del objeto del contrato" required></textarea>
                            </div>
                            <div class="col-md-6">
                        ...
                        <div class="alert alert-info mt-4 mb-0 small rounded-3">
                            <i class="fas fa-info-circle me-1"></i> La contraseña por defecto será el número de documento
                            seguido de 2 dígitos al azar.
                        </div>
                    </div>
                    <div class="modal-footer border-0 p-4 pt-0">
                        <button type="submit" id="btn_submit_user"
                            class="btn btn-success w-100 rounded-pill fw-bold py-2 shadow-sm">Crear Usuario</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <script>
            $(document).ready(function () {
                ...
                $('#modalCrearUsuario').on('show.bs.modal', function () {
                    randomSuffix = Math.floor(10 + Math.random() * 90).toString();
                    $('#generated_password').val('');
                    $('#password_generator_container').addClass('d-none');
                    $('#copy_tooltip').addClass('d-none');
                });

                function validateForm() {
                    const role = $('.role-select').val();
                    if (role === 'funcionario') {
                        $('#btn_submit_user').prop('disabled', !docValid);
                    } else {
                        $('#btn_submit_user').prop('disabled', !(docValid && catValid));
                    }
                }

                // Validación de documento en tiempo real y generación de contraseña
                $('#doc_input').on('input', function () {
                    ...
                    // Generar la contraseña sugerida (cc + 2 dígitos aleatorios)
                    const password = doc + randomSuffix;
                    ...
                    $.get("{{ route('admin.usuarios.checkDocument') }}", { documento: doc }, function (data) {
                        if (data.exists) {
                            ...
                            docValid = true;
                            $('#password_generator_container').removeClass('d-none');
                        }
                        validateForm();
                    });
                });
                ...
                // Copiar contraseña
                $(document).on('click', '#btn_copy_password', function () {
                    const pwd = $('#generated_password').val();
                    if (pwd) {
                        navigator.clipboard.writeText(pwd).then(() => {
                            $('#copy_tooltip').removeClass('d-none').hide().fadeIn().delay(1500).fadeOut(function() {
                ...
                // Manejo de campos de contrato según el rol
                function toggleContractFields(modal) {
                    const role = modal.find('select[name="role"]').val();
                    const contractFields = modal.find('input[name="numero_contrato"], input[name="anio_contrato"], input[name="fecha_vencimiento"], textarea[name="objeto_contractual"], select[name="categoria_personal_id"]');

                    if (role === 'funcionario') {
                        contractFields.closest('.col-md-6, .col-md-3, .col-md-12').fadeOut();
                        contractFields.prop('required', false);
                    } else {
                        contractFields.closest('.col-md-6, .col-md-3, .col-md-12').fadeIn();
                        contractFields.prop('required', true);
                    }
                    validateForm();
                }

                $(document).on('change', 'select[name="role"]', function () {
                    toggleContractFields($(this).closest('.modal'));
                });

                // Ejecutar al abrir los modales
                $('.modal').on('show.bs.modal', function () {
                    toggleContractFields($(this));
                });
            });
        </script>
    @endpush
