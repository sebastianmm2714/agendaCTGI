<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Facades\DB;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        DB::statement("ALTER TABLE users MODIFY COLUMN role ENUM('contratista', 'administrador', 'viaticos', 'supervisor_contrato', 'ordenador_gasto', 'funcionario') NOT NULL DEFAULT 'contratista'");
    }

    public function down(): void
    {
        DB::statement("ALTER TABLE users MODIFY COLUMN role ENUM('contratista', 'administrador', 'viaticos', 'supervisor_contrato', 'ordenador_gasto') NOT NULL DEFAULT 'contratista'");
    }
};
